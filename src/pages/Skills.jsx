...
export default function Skills() {
  ...
  return (
    <div style={{ width: '100%', maxWidth: '1000px', margin: '0 auto', paddingBottom: '50px' }}>
      <h2 style={{ fontSize: '2.8rem', textAlign: 'center', marginBottom: '40px', fontWeight: 800, color: 'var(--text-main)' }}>
        {translations.skills?.title || 'Teknik Yetenekler'}
      </h2>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '22px' }}>
              {cat.items.map((skill, i) => (
                <SkillBar
                  key={i}
                  skill={skill}
                  icon={iconMap[skill.icon] || skill.name[0]}
                />
              ))}
            </div>
          </div>
